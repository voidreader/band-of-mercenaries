...
M6 지명 의뢰(`is_named`)의 확장으로 `quest_pools`에 `party_size_min`/`party_size_max` 2개 INT 컬럼을 추가하고, 솔로(정확히 1인) 3종 + 소수정예(2인/3인) 2종 의뢰 5행을 삽입한다. `CombatSimulator`에 per-merc 사망 저항 cap 인자를 추가하고, `QuestCompletionService`에서 카운터 증가 + 칭호 hook 평가를 trailing fail-soft로 수행한다. `DispatchDetailPage`에서 파티 규모 강제 UI를 적용하며, 의뢰 카드에 ⭐/⭐⭐/⭐⭐⭐ 배지를 추가한다. 신규 Hive 박스·typeId 추가 없이 기존 M6 지명 의뢰 패턴과 M8b CombatSimulator를 최소 확장으로 재활용한다.
...
- **[FR-4]** `items` 2행 신규 INSERT — `guild_artifact_lone_wolf_compass` / `guild_artifact_three_kings_seal` (category=`guild_artifact`, slot=`guild_equipment`). **INSERT 자체는 본 명세 구현 범위이며, verifier는 Supabase `items` 테이블에 해당 2행 존재 여부로 PASS/FAIL 판정한다.** 효과 수치(`special_flags` JSONB 또는 모델 확장)는 페이즈 3 #4에서 확정한다 (현재는 `special_flags: {}` 빈 객체로 INSERT).
...
- **[FR-10]** `quest_provider.dart`의 `_applyCompletionResult()` — 4 카운터 증가 trailing fail-soft. **`QuestCompletionService`는 순수 계산 서비스 원칙을 유지하므로 카운터 영속화·hook 평가는 `quest_provider` 계층에 위치한다.** 기존 `region_N_dispatch_count` 갱신(line 1300~1318) 직후에 추가:
  ...
  try {
    if (pool != null &&
        ...
        (result.resultType == QuestResultType.success ||
         result.resultType == QuestResultType.greatSuccess)) {
      if (pool.partySizeMax == 1) {
        // 솔로: partyMercs.first만 (1인 파티)
        final merc = partyMercs.first;
        final updatedStats = Map<String, dynamic>.from(merc.stats);
      ...
      for (final merc in partyMercs) {
        await ref.read(titleServiceProvider).evaluateActionStatHook(merc);
      }
    }
  } catch (e) {
...
- **[FR-19]** 솔로 의뢰 성공·대성공 시 아이템 **보장 드랍** trailing fail-soft (ISSUE-1·5 해소):
  ...
  - **보장 드랍 처리**:
    ...
    try {
      ...
      if (guaranteedItemId != null &&
          (result.resultType == QuestResultType.success ||
           result.resultType == QuestResultType.greatSuccess)) {
        final alreadyHas = inventoryRepo.hasItem(guaranteedItemId);
        if (!alreadyHas) {
          inventoryRepo.addItem(guaranteedItemId);
          activityLog.add(ActivityLog(
            type: ActivityLogType.factionRewardGranted,  // 기존 재활용 또는 신규 enum 검토
            message: '솔로 의뢰 보상 — {item_name}(을)를 획득했다',
            timestamp: DateTime.now(),
          ));
        } else {
          // 중복 정책: 대체 골드 변환
          final goldAmount = 100 * (pool.difficulty ?? 1);
          userData.addGold(goldAmount);
          activityLog.add(ActivityLog(
            type: ActivityLogType.questResult,
            message: '솔로 의뢰 보상 — ${goldAmount}G로 변환되었다',
            timestamp: DateTime.now(),
          ));
        }
      }
  ...
  - **중복 정책**: `InventoryRepository.hasItem(itemId) == true`이면 대체 골드 `100G * pool.difficulty`. 재료 묶음 변환은 페이즈 3 #4에서 SQL 검토 위임.
    ...
    try {
      ...
      if (dropEntry != null &&
        ...
        if (rng.nextDouble() < dropEntry.chance) {
          inventoryRepo.addItem(dropEntry.itemId);
          activityLog.add(ActivityLog(
            type: ActivityLogType.factionRewardGranted,
            message: '솔로 의뢰 보상 — 희귀 아이템 {item_name}(을)를 획득했다',
            timestamp: DateTime.now(),
          ));
        }
      }
  ...
  - **처리 방식** (합리적 default — ActivityLog 단독):
    ...
    try {
      ...
      if (epilogue != null &&
        ...
        activityLog.add(ActivityLog(
          type: ActivityLogType.questResult,
          message: epilogue,  // '외로운 늑대의 이름이 또 한 번 입에 오르내렸다.'
          timestamp: DateTime.now(),
        ));
      }
    } catch (e) {
...
| Supabase `items` | 2행 INSERT (`guild_artifact_lone_wolf_compass` / `guild_artifact_three_kings_seal`) — 효과 수치 페이즈 3 #4 확정 |
