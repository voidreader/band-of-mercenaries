...
> 마지막 업데이트: 2026-05-19T11:00:00+09:00
> 현재 페이즈: 4 (완료)
> 상태: archived
> 종료: 2026-05-19T11:00:00+09:00
> 아카이브 경로: `Docs/Archive/20260519_m8a_faction_combat_report/`
...
- [x] 4. 통합 구현 명세 및 검증 계획
  - 입력 의존: 페이즈 4 산출물 1~3
  - 산출물: `Docs/spec/[spec]20260519_m8a-integration-and-verification.md`
  - 완료: 2026-05-19T10:30:00+09:00 (spec-pipeline PASS, revision 0회. 9/9 완료 기준 충족, Supabase 시드 검증 후 finalize-feature 완료)
...
- [x] 대표 세력 2~3개가 생활권 사건과 연결되어 등장한다.
- [x] 세력 지명 의뢰가 위업, 신뢰도, 칭호, 지역 상태 조건으로 노출된다.
- [x] 세력 상점 상품이 제작 재료 또는 레시피와 연결된다.
- [x] 세력 보상이 용병단 위상 또는 지역 상태에 영향을 준다.
- [x] 세력·지명·엘리트·연계 의뢰 결과에 전투 보고서가 저장된다.
- [x] 전투 보고서는 요약과 상세 로그를 구분하여 표시된다.
- [x] 전투 보고서는 완료 후 재접속해도 동일하게 유지된다.
- [x] 14세력 전체 확장을 전제로 하지 않아도 MVP가 완결된다.
- [x] M1~M7 기능 회귀 이상 없음 — `flutter analyze` 0 issues, `flutter test` 578 PASS.
...
- 2026-05-19T10:30:00+09:00: 페이즈 4 산출물 4 작성 완료 (`Docs/spec/[spec]20260519_m8a-integration-and-verification.md`). spec-pipeline PASS(revision 0회, Opus 검증 5/5 항목 모두 PASS). 페이즈 1~4 산출물 16개 통합 매트릭스 + 명세 vs 실제 구현 정합성 + M1~M7 회귀 결과 + M8a 9/9 완료 기준 평가 + CLAUDE.md 갱신 7 영역 + finalize-feature 인계 + M8b 잔여 백로그(Q-1 faction_* Supabase 시드 검증·Q-2 operation-bom 일괄 등록) 단일 문서 통합.
- 2026-05-19T10:50:00+09:00: FR-G1 Supabase 시드 검증 결과 페이즈 4 #1 마이그레이션 전체 미적용 발견 — `faction_contacts`/`faction_reactions`/`faction_shop_items` 3 테이블 부재, `quest_pools`/`items`/`crafting_recipes`/`titles` M8a 행 0건. 사용자 결정: 즉시 마이그레이션 SQL 작성·적용.
- 2026-05-19T10:55:00+09:00: M8a 페이즈 4 #1 Supabase 마이그레이션 5건 적용 완료 — `m8a_p4_1_faction_tables_schema`(3 테이블 + RLS + 인덱스 + CHECK 확장 2종) / `m8a_p4_1_seed_items_recipes_titles`(items 17 + recipes 2 + titles 2) / `m8a_p4_1_seed_faction_data`(contacts 3 + reactions 33 + shop_items 18) / `m8a_p4_1_seed_quest_pools`(quest_pools 12) / `m8a_p4_1_data_versions_publish`(data_versions 신규 3 + 기존 4 v+1). 검증 SQL 결과 모든 행 카운트 + FK 무결성 정합.
- 2026-05-19T11:00:00+09:00: finalize-feature 완료 — `Docs/Archive/20260519_m8a_faction_combat_report/`로 페이즈 1~4 산출물 26개 + state.md 이관. CLAUDE.md 7 영역 갱신(테이블 32→37·HiveField 표 5행 추가/수정·게임 핵심 시스템 로직 절 2 항목 추가·CraftingService unlock 6 type 명시·테스트 파일 수 갱신). CHANGELOG fragment 3개 생성. M8a 마일스톤 상태 `archived` 전환, 완료 기준 9/9 충족 확인.
