...
4. **코딩 에이전트 파이프라인 실행**
   ...
   **검증 모드 분기 (TASK 수 기준)**
   ...
   1. **main 직접 명세 검증**
      - coder가 보고한 모든 변경 파일을 Read로 읽어 실제 내용을 확인한다.
      - 명세서의 각 요구사항(REQ-n)이 코드에 구현되었는지 대조한다.
      - 계획 리포트의 시그니처(클래스명/함수명/타입)를 coder가 정확히 따랐는지 확인한다.
   ...
   **판정:**
   ...
   각 에이전트에 전달할 내용:
   - **verifier**: 원본 명세서 + 통합 계획 리포트 + 모든 coder 완료 보고 (변경 파일 목록 포함)
