...
- `settlementTrustLevelUpProvider`: 마을 신뢰도 단계 승급 이벤트 채널 StateProvider<TrustLevelUpEvent?> (`features/investigation/domain/trust_level_up_event.dart`). `RegionStateRepository.addSettlementTrust`가 단계 승급 시 publish, `app.dart`가 `ref.listen`으로 감지하여 `dialogQueueProvider.enqueue(high priority)`로 등록. `RankUpEvent` 패턴 모방
- `settlementTrustProvider`: 특정 regionId의 신뢰도/단계를 동기 조회하는 Provider.family<({int trust, int level}), int> (`features/investigation/domain/settlement_trust_provider.dart`). `regionStateRepositoryProvider` watch 후 `getSettlementTrust(regionId)` 반환. 단순 동기 조회용으로, RegionState 변경 시 자동 갱신은 별도 채널(`settlementTrustLevelUpProvider`) 활용
...
- quest_types / quest_pools: 퀘스트 유형 및 풀. `quest_pools` 세력 태그 확장 컬럼: `type_id`(text NOT NULL DEFAULT 'raid', 신규 유형 참조용), `faction_tag`(text nullable FK → factions.id, 전용 퀘스트 고정 세력), `is_faction_exclusive`(bool NOT NULL DEFAULT false), `min_reputation`(int NOT NULL DEFAULT 0, 전용 퀘스트 해금 임계 기본 11 / 고급 61), `sector_type`(text nullable, M3 대비 필드), `enemy_name`(text nullable, 퀘스트 서사 `{quest.enemy}` 치환용). 기존 `type` real 필드는 deprecated 유지. 세력 전용 퀘스트 98행(14세력 × 7개) 포함, 총 298행. **M4 페이즈 4 #3 거점 사건/허드렛일 확장 컬럼 9개 추가**: `is_fixed`(bool NOT NULL DEFAULT false, true=고정 의뢰로 일반 갱신 주기에서 제외), `fixed_chain_id`(text nullable, 고정 사건 라인 ID — `settlement_<region_id>_<event_name>` 컨벤션, 예 `settlement_3_pyegwang_reopen`), `fixed_step`(int nullable CHECK 1..20, 1-based), `trust_threshold`(int nullable CHECK 1..4, 노출 시작 신뢰도 단계), `reward_gold_override`/`reward_xp_bonus_override`/`duration_override_seconds`/`trust_reward_override`(int nullable, is_fixed=true 행 보상/시간/신뢰도 보상 override), `min_trust_level`(int NOT NULL DEFAULT 0, 일반 풀의 단계별 노출 제어 — 채집 의뢰는 2). Partial UNIQUE 인덱스 `(fixed_chain_id, fixed_step) WHERE is_fixed=true` 추가. M4 페이즈 4 #3 신규 INSERT 16행: `qp_pyegwang_step1~6` 거점 사건 라인 6단계(폐광길 재개방, explore→hunt→raid→escort→raid→survey, trust_threshold 1·1·2·2·3·3, duration_override 300·300·360·300·600·600s, trust_reward 10·15·20·25·30·100, gold_override step3+ 200·185·270·500G, step6 xp_bonus +50) + `dustvile_chore_01~10` 더스트빌 허드렛일 10건(labor 6+escort 1+explore 2+hunt 1, 모두 난이도 1, min_region_diff=1, max_region_diff=1, dustvile_chore_03만 min_trust_level=2 채집 의뢰). 본 페이즈는 `_getCurrentTrustLevel() = 0` stub 상태로 trust_threshold ≥ 1 조건 실패 → 고정 의뢰 미노출 안전 fallback (페이즈 4 #5 RegionStateRepository.getSettlementTrust 연결 시 활성화)
...
- `regionStates` 박스: RegionState 모델 (typeId:8) — regionId(int), knowledge(int 0~100), triggeredDiscoveries(List<String>), HiveField(3) `sectorChanges` (Map<String, String>, 변형된 섹터 맵 key=섹터인덱스 문자열 "0"~"9" / value="village"|"ruins"|"hidden", 기본값 {}). 리전당 최대 1섹터 변형 (MVP 제약). regionId 키로 저장. **M4 페이즈 4 #5 마을 신뢰도 확장**: HiveField(4) `settlementTrust` (int?, null=0 fallback, 누적 신뢰도 점수 0~999999 clamp), HiveField(5) `settlementTrustLevel` (int?, null=1 fallback, 1~4 단계 캐시). `currentTrust`/`currentTrustLevel` getter로 null-safe 접근. **M4 페이즈 4 #4 거점 사건 24h 윈도우**: HiveField(6) `lastEventCompletedAt` (DateTime?, 거점 사건 step==6 완료 시각. `quest_provider._applyCompletionResult` step==6 분기에서 `RegionStateRepository.setEventCompleted(regionId)` 호출하여 갱신). `eventCompletedRecently` getter로 24h 이내 여부 판정 — 거점 화면들이 사건 완료 메시지 우선 노출에 사용
...
- `quests` 박스 체인 퀘스트 확장: HiveField(20) `eliteId` (String?, 엘리트 몬스터 ID), HiveField(21) `isChainStep` (bool?), HiveField(22) `chainId` (String?), HiveField(23) `chainStep` (int?). `isChainQuest` getter(`isChainStep ?? false`), `isElite` getter(`eliteId != null`), `isSettlementStep` getter(`isChainQuest && (chainId?.startsWith('settlement_') ?? false)` — M4 페이즈 4 #3 거점 사건 step 판별). 체인 단계는 `QuestListNotifier.injectChainStep(ChainQuestData, userRegion)`으로 대기열에 삽입. 거점 사건(settlement_ prefix) step은 `QuestListNotifier._injectFixedSettlementQuest()`가 `chainQuestProgress.currentStep` + `quest_pools.trust_threshold` 조건 충족 시 `ActiveQuest(isChainStep=true, chainId='settlement_<region_id>_<event_name>', chainStep=currentStep)` 형태로 자동 주입
...
- `user` 박스 마을 방문: HiveField(22) `herbalistCooldownEndTime` (DateTime?, 약초상 즉시 회복 다음 사용 가능 시각. null=쿨다운 없음), HiveField(23) `lastSmithyRepairAt` (DateTime?, 낡은 대장간 수리 의뢰 마지막 사용 시각. 24h 쿨다운 판정용). `UserDataNotifier.setHerbalistCooldown(DateTime?)` / `setSmithyRepairAt(DateTime?)` 메서드로 영속
...
- `activityLogs` 박스 마을 신뢰도 확장: `ActivityLogType` enum HiveField(22) `settlementTrustUp` / HiveField(23) `settlementEventStep` / HiveField(24) `settlementEventCompleted` (M4 페이즈 4 #5)
- `activityLogs` 박스 마을 방문 확장: `ActivityLogType` enum HiveField(25) `herbalistHeal` / HiveField(26) `smithyRepairCompleted` (M4 페이즈 4 #4)
...
- **마을 신뢰도(거점 사건)**: M4 페이즈 4 #5 신규 시스템. 거점(MVP=region 3 더스트빌)에서 일반 의뢰·고정 사건 step 완료 시 `RegionStateRepository.addSettlementTrust(regionId, amount, source, ref)`이 누적 점수 증가 + 임계값 표(`{1:0, 2:30, 3:80, 4:200}`) 순회로 단계 승급 검증. 4단계명 "의심/인지/친근/소속". 단계 승급 시 통과한 모든 단계의 일회성 보상 합산 지급(2단계 +100G+50XP / 3단계 +200G+100XP / 4단계 +500G+200XP+100명성, XP는 살아있는 용병 균등 분배 + 잔여 0번째 가산) + `settlementTrustUp` ActivityLog + `settlementTrustLevelUpProvider` publish + `QuestListNotifier.refreshAvailableQuests()` 호출. 일반 의뢰 신뢰도 점수: region==3 + !isChainQuest + factionTag==null + 성공/대성공 시 난이도별 2/3/5/0/0점(`QuestCompletionResult.settlementTrustGain`). 거점 사건 step 완료 시 `quest_pools.trust_threshold` 조건 + `trust_reward_override` 기반 누적. 게임 시작(`UserDataNotifier.initializeNewGame`) + region 3 진입(`MovementNotifier._completeMovement`) 시 자동으로 RegionState 초기화 + `ChainQuestService.tryActivateSettlement(3, 'pyegwang_reopen')` 호출(멱등성 보장). 거점 사건은 `settlement_<regionId>_<eventName>` chainId 컨벤션으로 일반 체인과 분리: `checkDormant`/`onStepCompleted` protagonist resolution 모두 settlement_ prefix skip. `setSettlementTrust(regionId, trust, level)`은 운영/디버그 전용으로 일회성 보상 우회. 4단계 진입 시 `RankUpEvent`(critical) → `TrustLevelUpEvent`(high) → `ChainCompletedEvent`(high) 순으로 dialog 큐 직렬화
...
- **공존 정책 / 다이얼로그 큐**: M3 이후 5계층 컨텐츠(체인·세력 전용·엘리트·변형 섹터·일반)가 파견 화면에 공존, 한 번의 이동 도착 시 다중 팝업이 발생. `dialogQueueProvider` 단일 큐로 5개 도메인 채널(건설=medium / 조사=medium / 랭크업=critical / 체인 완주=high / 지역 변형=high) + 이동 자동 이벤트=medium / 선택지 회상=medium 통합. `app.dart`가 `_isShowingDialog` 플래그 + `mounted` 가드로 중복 표시 방지 + `addPostFrameCallback`으로 head 표시. critical은 `barrierDismissible: false`. 파견 화면 정렬은 `QuestSortService.sort()` 5계층 fold (Tier 0 체인 active → Tier 1 세력 전용 → Tier 2 엘리트(유니크 우선) → Tier 3 변형 섹터 → Tier 4 일반). 같은 tier는 `questType.baseReward × difficulty` 내림 → difficulty 오름 → id 사전순(pending 시 ActiveQuest.rewardGold가 null이라 추정값 사용). Tier 0는 `ChainTopSection`(최대 3장, 활성/비활성/0 케이스, 비활성은 opacity 0.6 + "이동 화면으로" 버튼으로 `currentTabProvider = 0`)으로 별도 렌더, 일반 목록에서 제거. 카드 시각 통합은 `LayerSidebar`(8단계 우선순위 fold) + `QuestCardBadges`(체인/엘리트/섹터/세력 4종 배지, 세력명 6자 초과 시 3자+"…"). MovementScreen은 `chainTargetSectors`(`Map<int, Set<int?>>`)로 region+sector 매칭하여 체인 대상 섹터 타일에 금색 2px 테두리 + "체인" 마이크로 배지 표시(`null` 포함 시 region 전체 fallback). AppTheme에 `chainGold`(0xFFD4AF37) 신규 + `transformVillage/Ruins/Hidden`, `eliteAccent/UniqueAccent` 명세 색상으로 갱신. **M4 페이즈 4 #3 거점 사건 분기**: `QuestSortResult`에 `settlementTier` 신규 필드 — `chainTier0` 분류 시 `chainId.startsWith('settlement_')` prefix는 `chainTier0`에서 제외하고 `settlementTier`로 분리. `sortedRest = [...settlementTier, ...tier1~4]` 순서로 일반 목록 최상단 배치. `dispatch_screen.dart`의 `_QuestCard`가 `quest.isSettlementStep` 분기로 "📜 마을 사건" 인라인 배지 추가(`AppTheme.settlementAccent` 0xFFFFA000 알파 0.15 배경 + 1px 테두리). 페이즈 4 #5에서 `ChainTopSection` `actives` 필터에 `!p.chainId.startsWith('settlement_')` 추가 완료 (거점 사건은 일반 목록의 settlementTier로만 노출)
- **마을 방문 (거점 3종 + 약초상)**: M4 페이즈 4 #4 신규 시스템. 시작 거점 더스트빌(region 3, sector 1)의 sector_type=='village' 진입 시 `MovementScreen` 하단에 마을 영역 인라인 노출(`VillageVisitSection`). `_selectedFacility` enum(`VillageFacility` chiefHouse/oldSmithy/herbalist) 분기로 거점 화면 상태 기반 렌더링(`Navigator.push` 미사용). 거점 화면 3종은 `features/settlement/view/` 신규 모듈에 위치. NPC 5명(파슨/하겐/네리스/도라/레미) + 단계별 인사말 + 광장 풍문 + 사건 완료 메시지는 `features/settlement/domain/settlement_npc_data.dart` const 인라인. 신뢰도 단계별 잠금 정책: 1단계 의심·2단계 인지·3단계 친근·4단계 소속. **약초상 1회성 회복**: `HerbalistService.calculateCost(int)` 75/50/45/40G + `calculateCooldownMinutes(int)` 45/30/15/10m + `gatheringMultiplier(int)` 1.0/1.0/1.1/1.2 (채집 의뢰 골드 배수). 의무실(`facilities['infirmary']`) 자동 회복과 동시 작동, 약초상 사용 시 `MercenaryRepository.healInstant`가 `injuryEndTime/tiredEndTime` 둘 다 null 처리하여 자동 타이머도 즉시 종료. `MercenaryListNotifier.healInstant({mercId, cost, cooldownMinutes})` wrapper가 spendGold + setHerbalistCooldown + ActivityLog(`herbalistHeal`) 일괄 처리. **채집 의뢰 골드 배수**: `QuestCompletionService.calculate(currentTrustLevel)` 시그니처 + `quest.questPoolId == 'dustvile_chore_03'` 단일 행 분기로 success/greatSuccess 시 배수 곱(quest_provider가 `regionStateRepository.getSettlementTrust(quest.region).level` 주입). 신뢰도 점수표는 페이즈 4 #5 결정값 그대로 유지. **낡은 대장간 [수리 의뢰]**: M4 MVP 인라인 stub — 1회/24h, 신뢰도 3단계 50G·4단계 60G(×1.2). `UserData.lastSmithyRepairAt` 영속 + `ActivityLogType.smithyRepairCompleted` 로그. **시간 미소모 원칙**: 거점 화면·즉시 회복·수리 의뢰 모두 `gameTickProvider`는 표시 갱신용이며 게임 시간을 소모하지 않는다.
