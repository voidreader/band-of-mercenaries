...
> 마지막 업데이트: 2026-04-16 (세력 핵심 시스템 구현 반영)
...
| 패시브 혜택 | ⚠️ | `passive_bonus_json` 텍스트 표시만, 실제 게임 효과 미연동 (stub) |
...
| 세력 패시브 효과 실제 연동 | 높음 | `passive_bonus_json` 기반 실제 게임 효과 반영 (현재 텍스트 표시만) |
| 세력 평판 획득 트리거 | 중 | 퀘스트 완료 / 이벤트 시 세력 평판 변동 자동 반영 (현재 UI에서만 조작 가능) |
| 세력 전용 의뢰 | 중 | 가입 세력 전용 퀘스트 풀 및 보상 |
...
```
lib/
├── core/models/        18개 정적 데이터 모델 (Freezed) — TraitData, TraitCategory, TraitConflict, TraitTransition, TraitComboEvolution, TraitSynergy, RegionDiscovery, FactionData 포함
...
│   │   └── view/       InfoScreen, FactionCodexScreen, FactionDetailScreen
│   └── settings/       SettingsScreen (개발 도구만 유지, 홈 탭으로 이동)
└── shared/widgets/     BottomNavBar (6탭), TimerDisplay, StatusBadge
```

서비스 14개: ReputationService, TravelEventService, ExperienceService, QuestCalculator, RecruitmentService, FacilityService, **ConstructionService**, MercenaryStatService, TraitEffectService, TraitAcquisitionService, TraitEvolutionService, TraitDeletionService, **InvestigationService**, **FactionJoinService**
